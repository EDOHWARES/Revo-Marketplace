...
export function ProductFilters({ onFilterChange, categories, farmingMethods }: ProductFiltersProps) {
  ...
  const handleMouseMove = useCallback((event: MouseEvent) => {
    ...
    setLocalFilters(prev => {
      ...
      if (isDragging.current === 'min' && value <= max) {
        return { ...prev, priceRange: [value, max] };
      } else if (isDragging.current === 'max' && value >= min) {
        return { ...prev, priceRange: [min, value] };
      }
      return prev;
    });
  }, [maxPrice]);
  ...
  const hasActiveFilters = useMemo(() => {
    return (
      ...
      localFilters.farmingMethod !== '' ||
      localFilters.deliveryOnly ||
      localFilters.pickupOnly ||
  ...
  return (
    <div className="bg-white p-6 border border-gray-200 rounded-[20px]">
      <div className="flex justify-between items-center mb-4 border-b border-gray-200 pb-4">
        <h2 className="text-lg font-semibold">{t('filters.label')}</h2>
      ...
      <div className="space-y-4 border-b border-gray-200 pb-4 mb-4">
        {categories.map((category) => (
          <div
            key={category}
            className="flex justify-between items-center w-full py-2 px-1 hover:bg-gray-50 cursor-pointer"
          ...
          >
            ...
            <svg
              className={`h-5 w-5 transition-transform ${localFilters.category === category ? 'rotate-90' : ''
                }`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-xl font-medium">{t('filters.price')}</h3>
        </div>

        <div
          ref={sliderRef}
          className="relative h-1 bg-gray-200 rounded-full mt-6 mb-2 cursor-pointer mx-2"
        >
          <div
            className="absolute h-full bg-green-800 rounded-full transition-all duration-150"
            style={{
          ...
          />
          <div
            className="absolute w-4 h-4 bg-green-800 rounded-full -mt-1.5 -ml-2 cursor-grab active:cursor-grabbing shadow-md transition-transform hover:scale-110"
            style={{ left: `${(localFilters.priceRange[0] / maxPrice) * 100}%` }}
            onMouseDown={(e) => handleMouseDown(e, 'min')}
          />
          <div
            className="absolute w-4 h-4 bg-green-800 rounded-full -mt-1.5 -ml-2 cursor-grab active:cursor-grabbing shadow-md transition-transform hover:scale-110"
            style={{ left: `${(localFilters.priceRange[1] / maxPrice) * 100}%` }}
        ...
        <div className="flex items-center justify-between mt-2 text-gray-600">
          <div className="flex items-center gap-1">
            <span className="text-sm text-filter_active">$</span>
            <span className="text-sm font-medium text-filter_active">{localFilters.priceRange[0]}</span>
          </div>
          <div className="flex items-center gap-1">
            <span className="text-sm text-filter_active">$</span>
            <span className="text-sm font-medium text-filter_active">{localFilters.priceRange[1]}</span>
          </div>
        </div>
      </div>

      <div className="mt-2 border-b border-gray-200 pb-4"></div>
      <div className="mt-2 border-b border-gray-200 pb-4"></div>

      <button
        onClick={handleApplyFilters}
        className="w-full mt-6 bg-filter_active py-2 px-4 rounded-[8px] hover:bg-filter_active transition-colors"
      >
        <span className="text-white-dark">{t('filters.apply')}</span>
      </button>
      {hasActiveFilters && activeFilters && (
        <button
          onClick={handleClearFilters}
          className="w-full text-white mt-2 bg-clear_filter py-2 px-4 rounded-[8px] hover:bg-clear_filter transition-colors"
        >
          <span className="text-white-dark">{t('filters.clear')}</span>
        </button>
      )}
    </div>
  );
