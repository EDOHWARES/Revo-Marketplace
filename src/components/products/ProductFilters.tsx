...
export function ProductFilters({ onFilterChange, categories, farmingMethods }: ProductFiltersProps) {
  ...
  // Debounce la actualización del filtro para evitar demasiadas actualizaciones
  ...
  // Efecto para actualizar los filtros cuando cambia el rango
  useEffect(() => {
    debouncedFilterUpdate(priceRange);
    return () => debouncedFilterUpdate.cancel();
  }, [priceRange, debouncedFilterUpdate]);
  ...
  return (
    <div className="bg-white rounded-lg p-6">
      ...
      <div className="space-y-4 border-b border-gray-200 pb-4">
        {categories.map((category) => (
          ...
          >
            ...
            <svg
              className={`h-5 w-5 transition-transform ${filters.category === category ? 'rotate-90' : ''
                }`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
      ...
      <div className="space-y-4">
        ...
        <div 
          ref={sliderRef}
          className="relative h-1 bg-gray-200 rounded-full mt-6 mb-2 cursor-pointer"
        >
          {/* Barra de fondo */}
      ...
      <button
        ...
        className="w-full mt-6 bg-green-800 py-2 px-4 rounded-[8px] hover:bg-green-700 transition-colors"
      >
        <span className="!text-white">Apply Filter</span>
      </button>
    </div>
  );
} 